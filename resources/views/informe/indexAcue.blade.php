                       ...
                       <div class="card-body">
                        <div class="form-group row">
                            <div class="col-md-6">
                                ...
                                <div class="float-end">
                                    <a href="{{URL::action('App\Http\Controllers\InformeController@pdfResumenAcue')}}" target="_blank">
                                         ...
                                         @foreach($cuotas as $c)
                                            ...
                                            <tr>                                    
                                                ...
                                                @if($c->moneda == "GS")
                                                    ...
                                                    <td>Gs. {{number_format(($c->saldo_ven - $c->pagos_ven), 0, ",", ".")}}</td> 
                                                    @else
                                                    <td>U$S. {{number_format(($c->deuda), 0, ",", ".")}}</td>
                                                    <td>U$S. {{number_format(($c->pagos), 0, ",", ".")}}</td>
                                                    <td>U$S. {{number_format(($c->deuda - $c->pagos), 0, ",", ".")}}</td>
                                                    <td>U$S. {{number_format(($c->saldo_ven - $c->pagos_ven), 0, ",", ".")}}</td>
                                                @endif
                                                @php            
                                                    $atraso=0;
                                                @endphp
                                                @foreach($cuotasatrasadas as $ct)   
                                                    @if($c->acuerdo_id == $ct->acuerdo_id)  
                                                        @php            
                                                            $atraso=$ct->cuotasAt;
                                                        @endphp   
                                                    @endif
                                                ...
                                                <td>{{$atraso}}</td>
                                                <td>                                     
                                                    <a href="{{URL::action('App\Http\Controllers\AcuerdoController@detalleCuotasAcue', $c->acuerdo_id)}}">
                                                        <button type="button" class="btn btn-success btn-sm" >
